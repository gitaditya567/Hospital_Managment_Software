...
export function HospitalAdminDashboard() {
  ...
  const revenueShare = [
    { label: 'OPD & Clinical Fees', value: baseRevenue, color: '#3b82f6' }, // Blue
    { label: 'Pharmacy Sales', value: pharmacyRevenue, color: '#10b981' } // Green
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between bg-gradient-to-r from-blue-900 via-slate-800 to-slate-900 p-6 rounded-2xl shadow-lg border border-slate-700/50 relative overflow-hidden">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-emerald-400 uppercase tracking-widest">Active Node Operational</span>
          </div>
        ...
        <div className="mt-4 md:mt-0 flex items-center gap-3 text-xs bg-slate-900/60 p-3 rounded-xl border border-slate-700/60 text-slate-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Today's Revenue */}
        <div className="bg-gradient-to-br from-blue-50 to-white dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-2xl shadow-sm border border-blue-100 dark:border-slate-700/50 flex items-center justify-between transition-all hover:shadow-md hover:scale-[1.01] duration-200">
          <div className="space-y-2">
            <p className="text-xs font-bold text-blue-700 dark:text-blue-400 uppercase tracking-wider">Today's Revenue</p>
          ...
          <div className="h-12 w-12 rounded-xl bg-blue-500/10 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400 flex items-center justify-center border border-blue-200/40 dark:border-blue-700/30">
            <IndianRupee size={22} />
          </div>
        </div>

        {/* Total OPD Patients Today */}
        <div className="bg-gradient-to-br from-emerald-50 to-white dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-2xl shadow-sm border border-emerald-100 dark:border-slate-700/50 flex items-center justify-between transition-all hover:shadow-md hover:scale-[1.01] duration-200">
          <div className="space-y-2">
            <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">OPD Check-Ins Today</p>
            <h3 className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">
              {totalPatientsToday} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">Registered</span>
            </h3>
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-semibold">
              Live registration queues online
            </p>
          </div>
          <div className="h-12 w-12 rounded-xl bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 flex items-center justify-center border border-emerald-200/40 dark:border-emerald-700/30">
            <Activity size={22} />
          </div>
        </div>

        {/* Active Doctors Online */}
        <div className="bg-gradient-to-br from-purple-50 to-white dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-2xl shadow-sm border border-purple-100 dark:border-slate-700/50 flex items-center justify-between transition-all hover:shadow-md hover:scale-[1.01] duration-200">
          <div className="space-y-2">
            <p className="text-xs font-bold text-purple-700 dark:text-purple-400 uppercase tracking-wider">Active Doctors Online</p>
            <h3 className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">
              {activeDoctors} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">Active</span>
            </h3>
            <p className="text-[10px] text-purple-600 dark:text-purple-400 font-semibold">
              Assigned to active room schedules
            </p>
          </div>
          <div className="h-12 w-12 rounded-xl bg-purple-500/10 text-purple-600 dark:bg-purple-500/20 dark:text-purple-400 flex items-center justify-center border border-purple-200/40 dark:border-purple-700/30">
            <Users size={22} />
          </div>
        </div>

        {/* Pending Pharmacy Bills */}
        <div className="bg-gradient-to-br from-orange-50 to-white dark:from-slate-800 dark:to-slate-800/80 p-5 rounded-2xl shadow-sm border border-orange-100 dark:border-slate-700/50 flex items-center justify-between transition-all hover:shadow-md hover:scale-[1.01] duration-200">
          <div className="space-y-2">
            <p className="text-xs font-bold text-orange-700 dark:text-orange-400 uppercase tracking-wider">Pending Pharmacy Bills</p>
            <h3 className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">
              {pendingBillsCount} <span className="text-xs font-normal text-slate-500 dark:text-slate-400">Unpaid</span>
            </h3>
            <p className="text-[10px] text-orange-600 dark:text-orange-400 font-semibold flex items-center gap-0.5">
              Requires POS collection checkout
            </p>
          </div>
          <div className="h-12 w-12 rounded-xl bg-orange-500/10 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400 flex items-center justify-center border border-orange-200/40 dark:border-orange-700/30">
            <CreditCard size={22} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 flex flex-col justify-between">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-base font-bold text-slate-800">Patient Footfall</h2>
            ...
            <div className="flex gap-2 text-[10px] font-bold text-slate-600">
              <span className="flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-blue-500"></span> OPD Registrations
              </span>
            </div>
          </div>
              ...
              {points.map((p, idx) => (
                <g key={idx} className="group/dot">
                  <circle
                    ...
                    fill={idx === points.length - 1 ? '#3b82f6' : '#ffffff'}
                    stroke="#3b82f6"
                    strokeWidth={2.5}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 p-5 flex flex-col justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-800">Revenue by Department</h2>
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between sm:items-center gap-3">
          <div className="space-y-0.5">
            <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              <Clock className="text-blue-500" size={20} /> Today's Patient Check-Ins
            </h2>
              ...
              {recentPatients.map((patient) => {
                ...
                return (
                  <tr key={patient.id} className="hover:bg-slate-50/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-500 font-mono">{patient.phone}</td>
                    <td className="px-6 py-4 flex items-center gap-2 mt-2">
                      <div className="h-7 w-7 rounded-full bg-blue-50 text-blue-600 border border-blue-100 flex items-center justify-center font-bold text-[10px]">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded-md border border-blue-100/50 font-bold font-mono">
                        {patient.tokenNumber}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold ${
                        ...
                        isConsulting ? 'bg-indigo-50 text-indigo-700 border border-indigo-200/50' : 
