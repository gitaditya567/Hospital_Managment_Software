...
import { Search, Printer, Pill, X, AlertTriangle, AlertOctagon, Terminal } from 'lucide-react';
...
export function PharmacyDashboard() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto animate-in fade-in duration-200">
      ...
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between bg-gradient-to-r from-blue-900 via-slate-800 to-slate-900 p-6 rounded-2xl shadow-lg border border-slate-700/50 relative overflow-hidden">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-emerald-400 uppercase tracking-widest">Pharmacy POS Terminal</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">Point of Sale (POS)</h1>
          <p className="text-slate-300 text-sm">Process digital prescriptions instantly and maintain ACID database compliance.</p>
        </div>
      </div>

      {/* Hero Search Bar */}
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/80 max-w-3xl mx-auto">
        <form onSubmit={handleSearch} className="relative">
          ...
          <input
            ref={searchInputRef}
            type="text"
            className="block w-full pl-12 pr-28 py-3.5 text-lg font-bold font-mono uppercase bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/15 focus:border-blue-500 focus:bg-white transition-all shadow-inner placeholder-slate-400"
          ...
          <button
            type="submit"
            className="absolute inset-y-2 right-2 px-6 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold text-sm shadow-md transition-colors"
          >
            Load RX
          </button>
        </form>
      ...
      {activePrescription ? (
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-6 min-h-[400px] pb-24 animate-in slide-in-from-bottom-6 duration-300">
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
              <div className="p-4 bg-slate-50 border-b border-slate-100">
                <h3 className="font-extrabold text-slate-800 text-sm">Patient Context</h3>
              </div>
              <div className="p-5 space-y-4 text-sm font-semibold">
                <div>
                  <label className="text-[10px] uppercase text-slate-400 font-extrabold">RX Code Reference</label>
                  <p className="text-blue-600 font-mono font-extrabold text-lg mt-0.5">{activePrescription.id}</p>
                </div>
                {activePrescription.patientNo && (
                  <div>
                    <label className="text-[10px] uppercase text-slate-400 font-extrabold">Patient No (UID)</label>
                    <p className="text-blue-600 font-mono font-bold mt-0.5">{activePrescription.patientNo}</p>
                  </div>
                )}
                <div>
                  <label className="text-[10px] uppercase text-slate-400 font-extrabold">Patient Name</label>
                  <p className="text-slate-800 text-base font-extrabold mt-0.5">{activePrescription.patientName}</p>
                </div>
                <div>
                  <label className="text-[10px] uppercase text-slate-400 font-extrabold">Contact Phone</label>
                  <p className="text-slate-600 font-mono mt-0.5">{activePrescription.patientPhone}</p>
                </div>
                <div>
                  <label className="text-[10px] uppercase text-slate-400 font-extrabold">Prescribing Physician</label>
                  <p className="text-slate-800 mt-0.5">{activePrescription.doctorName}</p>
                </div>
            ...
            <div className="bg-slate-900 text-slate-200 rounded-2xl border border-slate-850 shadow-md p-4 flex-1 flex flex-col min-h-[220px]">
              <div className="flex items-center justify-between pb-2 border-b border-slate-800 mb-3 shrink-0">
                <h3 className="text-xs font-extrabold tracking-wider text-slate-400 uppercase flex items-center gap-1.5">
                  <Terminal size={14} className="text-blue-400" /> ACID Mongoose Transaction
                </h3>
          ...
          <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200/80 shadow-sm flex flex-col overflow-hidden">
            <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0">
              <h3 className="font-extrabold text-slate-800 text-sm">Smart Prescription Details</h3>
                  ...
                  {activePrescription.items.map((item: any) => {
                    ...
                    return (
                      <tr key={item.medicineId} className={`transition-colors ${
                        isOutOfStock ? 'bg-rose-50/70 hover:bg-rose-50' : 'hover:bg-slate-50/50'
                      }`}>
                        ...
                        <td className="px-5 py-3.5 text-center w-24">
                          <input 
                            ...
                            className="w-16 px-2 py-1 bg-slate-50 border border-slate-200 rounded-lg text-center font-bold font-mono text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none"
                          />
                        </td>
                        <td className="px-5 py-3.5 text-center">
                          {isOutOfStock ? (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-rose-100 text-rose-700 rounded-full text-[10px] font-bold border border-rose-200">
                              <AlertTriangle size={10} /> Stock Short ({item.stockAvailable} Left)
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-[10px] font-bold border border-emerald-250">
                        ...
                        <td className="px-5 py-3.5 text-right text-blue-600 font-mono font-extrabold">
                        ...
                        <td className="px-5 py-3.5 text-center">
                          <button
                            ...
                            className="p-1 hover:bg-slate-100 text-slate-400 hover:text-rose-600 rounded-lg transition-colors"
                          >
                            <X size={14} />
                          </button>
                        </td>
          ...
          <div className="fixed bottom-0 left-0 right-0 md:left-64 h-20 bg-white border-t border-slate-200 shadow-2xl flex items-center justify-between px-6 z-40 transition-all">
            <div className="flex items-center gap-8">
              <div>
                <span className="text-[10px] text-slate-400 uppercase font-extrabold tracking-wider block">Grand Total</span>
                <span className="text-2xl font-extrabold text-blue-600 font-mono">₹{grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              </div>
              
              <div className="flex items-center gap-2">
                <span className="text-xs text-slate-500 font-bold">Payment:</span>
                <select 
                  className="h-9 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-600"
                  value={paymentMode}
            ...
            <Button
              ...
              className="h-12 px-8 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold flex items-center gap-2.5 shadow-lg shadow-emerald-600/10 hover:-translate-y-[1px] transition-all disabled:opacity-50 disabled:-translate-y-0 disabled:shadow-none"
            >
        ...
        <div className="h-80 flex flex-col justify-center items-center text-center p-8 bg-white border border-slate-200/80 rounded-2xl shadow-sm border-dashed">
          <div className="h-14 w-14 rounded-full bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 text-xl font-extrabold mb-3 shadow-inner">
            💊
          </div>
          <h3 className="font-extrabold text-slate-800 text-base">POS Ready to Scan</h3>
          <p className="text-slate-500 text-xs mt-1 max-w-sm leading-relaxed">
            Enter doctor prescription codes to review medication stocks and dispense items instantly. (Try entering <span className="font-mono font-bold text-blue-600">RX-9042</span> or <span className="font-mono font-bold text-blue-600">9876543210</span>)
          </p>
        </div>
      )}
