...
export function SuperAdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 p-6 rounded-2xl border border-slate-800 text-white shadow-xl">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">SaaS Central Command</h1>
          <p className="text-slate-400 text-xs mt-1">Multi-tenant hospital directory, secure manual licenses & platform telemetry</p>
        </div>
        <div className="flex gap-2 shrink-0">
          <Button 
            onClick={() => setLicenseModalOpen(true)}
            className="bg-blue-600 hover:bg-blue-700 text-white border-0 shadow-lg shadow-blue-500/20 text-xs font-semibold"
          >
            + Generate License
          </Button>
          <Button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard 
          ...
          trendUp={true} 
          className="border-slate-100 hover:shadow-md hover:border-slate-200 transition-all duration-200"
        />
        <StatCard 
        ...
        <StatCard 
          ...
          trendUp={false} 
          className={expiringSoonCount > 0 ? "border-rose-100 bg-rose-50/30 hover:shadow-md transition-all duration-200" : "border-slate-100 hover:shadow-md transition-all duration-200"}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-base font-bold text-slate-800">SaaS Growth Telemetry</h2>
          ...
          <div className="h-64 w-full relative">
            <svg viewBox="0 0 700 240" className="w-full h-full overflow-visible">
              ...
              {/* Grid Lines */}
              <line x1="0" y1="40" x2="700" y2="40" stroke="#F1F5F9" strokeWidth="1" strokeDasharray="3"/>
              <line x1="0" y1="100" x2="700" y2="100" stroke="#F1F5F9" strokeWidth="1" strokeDasharray="3"/>
              <line x1="0" y1="160" x2="700" y2="160" stroke="#F1F5F9" strokeWidth="1" strokeDasharray="3"/>
              <line x1="0" y1="220" x2="700" y2="220" stroke="#E2E8F0" strokeWidth="1"/>
              ...
              {/* Data points */}
              <circle cx={lastMRRPoint.x} cy={lastMRRPoint.y} r="5" fill="#2563EB" stroke="#FFFFFF" strokeWidth="2" />
              <circle cx={lastClinicPoint.x} cy={lastClinicPoint.y} r="4" fill="#6366F1" stroke="#FFFFFF" strokeWidth="1.5" />
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-800">SaaS Activity Log</h2>
          ...
          <Button 
            ...
            className="flex items-center gap-1.5 border-slate-200 text-slate-600 hover:bg-slate-50 text-xs"
          >
        ...
        <div className="divide-y divide-slate-50">
          {recentActivities.map((act) => (
            <div key={act.id} className="p-4 hover:bg-slate-50 transition-colors flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 ${
                  act.type === 'license' ? 'bg-amber-50 text-amber-600' :
                  act.type === 'payment' ? 'bg-emerald-50 text-emerald-600' :
                  act.type === 'hospital' ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-600'
                }`}>
              ...
              <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                act.type === 'license' ? 'bg-amber-100 text-amber-700' :
                act.type === 'payment' ? 'bg-emerald-100 text-emerald-700' :
                act.type === 'hospital' ? 'bg-blue-100 text-blue-700' : 'bg-slate-200 text-slate-700'
              }`}>
                {act.type}
              </span>
            </div>
      ...
      <Modal isOpen={isLicenseModalOpen} onClose={handleCloseModal} title="Generate License Key">
        <div className="space-y-4">
          ...
          {!generatedCode ? (
            <Button onClick={handleGenerate} className="w-full mt-4 bg-blue-600 hover:bg-blue-700 shadow-md">
